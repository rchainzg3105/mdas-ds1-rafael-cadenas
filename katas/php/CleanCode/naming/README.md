...
```php
$d = 0; // ❌ ¿Qué es 'd'?
$userList = new stdClass(); // ❌ No es una List
$yyyymmdstr = "20251102"; // ❌ Impronunciable
$strFirstName = "John"; // ❌ Codificación innecesaria
...
- **Información falsa**: `$userList` es un objeto, no una List
- **Impronunciable**: `$yyyymmdstr` es difícil de leer y pronunciar
- **Codificaciones**: `$strFirstName` - PHP ya conoce el tipo
- **Números mágicos**: `18` - ¿qué significa este número?

## 🔧 Tu tarea

1. **Estudia** `naming-bad.php` - identifica nombres poco claros y magic numbers
2. **Implementa** tu solución en `naming-exercise.php` antes de ver la propuesta
3. **Observa** `naming-good.php` y compara con tu solución
