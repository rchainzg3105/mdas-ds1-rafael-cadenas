...
```php
public function processOrder($name, $email, $items, $payment) {
  // ❌ Valida
...
1. **Estudia** `functions-bad.php` - identifica funciones grandes con múltiples responsabilidades
2. **Implementa** tu solución en `functions-exercise.php` antes de ver la propuesta
3. **Observa** `functions-good.php` y compara con tu solución
